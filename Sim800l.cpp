...
String Sim_Status[] = { "Not Connected,Not Searching",
...
bool Sim800l::begin(){
  ...
  if (_readSerial().indexOf("OK")!=-1 ) 
     ...
     Serial.println(F("Error in Connecting GSM Module "));
   SIM.print(F("AT+CREG?\r\n"));
  _buffer = _readSerial();
  uint8_t idx = _buffer[(_buffer.indexOf("+CREG: 0,"))+9]&0x0f;
  Serial.print("SIM ");
  Serial.println(Sim_Status[idx]);
...
uint8_t Sim800l::SmsIndication(){
   uint8_t idx;
   _buffer = _readSerial();
   idx = _buffer.indexOf("+CMTI: \"SM\",");
   if (idx != -1) 
   {
      return _buffer[idx+12];
   }
   else
   {
     return 0;
